...
FSlateBrush UListDataObject_KeyRemap::GetIconFromCurrentKey()
{
	...
	{
		Debug::Print(
			TEXT("Unable to find an icon from the key") +
			GetOwningKeyMapping()->GetCurrentKey().GetDisplayName().ToString() +
			TEXT(" empty brush has applied.")
		);
	}
...
bool UListDataObject_KeyRemap::TryResetBackToDefaultValue()
{
	if (CanResetBackToDefaultValue())
	{
		...
		FKey DefaultKey = GetOwningKeyMapping()->GetDefaultKey(); 

		FMapPlayerKeyArgs KeyArgs;
		KeyArgs.MappingName = CachedOwningMappingName; 
		KeyArgs.Slot = CachedOwningMappableKeySlot; 
		KeyArgs.NewKey = DefaultKey;
